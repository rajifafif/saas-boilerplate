...
const { storeRole, fetchPermissions, loadingPermissions, permissions } = useRoles();
...
const formatLabel = (permName: string, groupName: string) => {
    // Remove group name from permission (e.g. 'member.create' -> 'Create')
    const lowerGroup = groupName.toLowerCase();
    let label = permName.replace(lowerGroup + '.', '');
    label = label.replace(lowerGroup + '_', ''); // Handle snake_case if present
...
const permissionGroups = computed(() => {
    ...
    if (searchQuery.value) {
        ...
        Object.keys(groups).forEach(key => {
            ...
            if (groupMatches) {
                // If group matches, keep all permissions? Or just show the group?
                // Let's keep the whole group if the group name matches
            } else if (filteredPerms.length > 0) {
                 // If permissions match, keep only matching permissions
...
const submit = async () => {
    ...
    const { error } = await storeRole(form.value);
    
    if (error.value) {
        errorMsg.value = error.value.data?.message || 'Failed to create role';
    } else {
  ...
  <div>
    <VCard class="pa-6">
      <div class="d-flex align-center mb-6">
        ...
        <h2 class="text-h5">Create Role</h2>
      </div>
      ...
      <VForm @submit.prevent="submit">
        ...
        <div v-if="loadingPermissions" class="d-flex justify-center pa-8">
        ...
        <div class="d-flex justify-end gap-4">
          ...
          <VBtn color="primary" type="submit" :loading="loading" size="large">Create Role</VBtn>
        </div>
      </VForm>
    </VCard>
  </div>
