...
const { userData, isLoggedIn, loggingOut } = useUserData()

const { signOut } = useAuth()

async function logout() {
  try {
    // console.log('asdasdadseeeeee')
...
const userProfileList = [
  { type: 'divider' },
  { type: 'navItem', icon: 'tabler-user', title: 'Profile', to: { name: 'manage-account-settings-tab', params: { tab: 'account' } } },
...
<template>
  ...
  >
    ...
    >
      <VImg
        v-if="false && userData && userData.avatar"
        :src="userData.avatar"
      />
      <VIcon
        v-else
